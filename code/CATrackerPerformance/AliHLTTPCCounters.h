...
//     AddCounter("extra_sec"       ,"ExtraSec  efficiency");
    ...
    //
    AddCounter("loopers"               ,"Loopers           efficiency");
...
//  };
  ...
  void IncA( int _nhits, float _purity ) {
    reco.counters[indices["nhits"]] += _nhits;
    reco.counters[indices["purity"]] += _purity*100.;
    reco.counters[indices["all_"]]++;

    tot_pur += _purity;
//    std::cout<<" ------- nTracks: "<<reco.counters[indices["all_"]]<<";   purity: "<<reco.counters[indices["purity"]]<<";   tot_pur: "<<tot_pur<<"\n";
  }
...
//    cout << ">Purity per track : " << double(reco.counters[indices["purity"]])/double(reco.counters[indices["total"]] + ghosts + clone.counters[indices["total"]]) << endl;
    ...
    cout << ">Purity per track : " << double(reco.counters[indices["purity"]]/100.)/double(reco.counters[indices["all_"]]) << endl;
...
class AliHLTTPCCAPerformanceMCTrackData{
  public:
    ...
    void SetAsLooper() { isLooper = true; }
    bool IsLooper() { return isLooper; }
    bool IsReconstructedLooper() { return nReconstructedLooper >= 1; }
    void AddReconstructedLooper() { nReconstructedLooper++; }
    void SetSetLooper(int set_) { set_looper = set_; }
    int  GetSetLooper() { return set_looper;}
    int GetNClonesLooper() { return (nReconstructedLooper > 1) ? nReconstructedLooper - 1 : 0; }
...
class AliHLTTPCCAPerformanceRecoTrackData{
  public:
    ...
    void SetLooper( int lp) { looper = lp; }
    int IsLooper() const { return looper; }
    int IsLooper() { return looper; }
