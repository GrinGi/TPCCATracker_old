...
{
  fGB = GBTracker;
  std::cout<<" --- SetGB. NHits: "<<GBTracker->NHits()<<";   NTracks: "<<GBTracker->NTracks()<<"\n";
}
...
void AliHLTTPCCADisplay::DrawPointGlobal( float x, float y, float z, int Start, float width )
{
  fMarker.SetMarkerSize( width );
  fMarker.SetMarkerColor( 2 +  Start);
...
void AliHLTTPCCADisplay::SetSliceGlobal( AliHLTTPCCATracker * slice )
{
  fSlice = slice;
  fSliceView = 0;
  const int halfNumberOfSlices = AliHLTTPCCAParameters::NumberOfSlices/2;
...
//  v.RotateX(TMath::Pi()/fAngleRotateX);
//  v.RotateZ(TMath::Pi()/fAngleRotateZ);
  v.RotateX(TMath::Pi()/70);
  v.RotateZ(TMath::Pi()/12);
  vx = v.X()*zoom;
  vy = v.Z()*zoom;
  ...
  vx *= (z0_zoom-gz)/z0_zoom * z_zoom;
  vy *= (z0_zoom-gz)/z0_zoom * z_zoom;
#endif
...
void AliHLTTPCCADisplay::DrawHitGlobal( float x, float y, float z, int color, float width )
{
  static TMarker m;
  m.SetMarkerStyle(20);
  m.SetMarkerColor( color );
  m.SetMarkerSize ( width );
...
//  const float z0_zoom = 1000.;
//  double vz;
//  TVector3 v(hit.gx, hit.gz, hit.gy);
//
////  v.RotateX(TMath::Pi()/fAngleRotateX);
////  v.RotateZ(TMath::Pi()/fAngleRotateZ);
//  v.RotateX(TMath::Pi()/70);
...
//  *vza = -v.Y()*zoom;
//
//  *vxa *= (z0_zoom-hit.gz)/z0_zoom * z_zoom;
//  *vya *= (z0_zoom-hit.gz)/z0_zoom * z_zoom;
//#endif
  } // for ih
...
void AliHLTTPCCADisplay::TDrawTrack::DrawGlobalZ()
{
  const int NHits = hits.size();
  if ( NHits < 2 ) return;
  if ( ! hits[0].CheckGlobal() ) FillGBHitsCoor();
...
void AliHLTTPCCADisplay::SpecDrawHitGlobal( float X, float Y, float Z, int iSlice, int color, float width )
{
  TDrawHit dhit;
  dhit.x = X;
  dhit.y = Y;
...
void AliHLTTPCCADisplay::SpecDrawLine( float x1, float y1, float z1, int slice1, float x2, float y2, float z2, int slice2, int color, float width )
{
  TDrawTrack drawTrack;
  TDrawHit dhit;
  dhit.x = x1;
  ...
  dhit.y = y2;
  dhit.z = z2;
  dhit.alpha = TDrawHit::GetAlpha( fGB, slice2 );
  dhit.SetColor(color);
  dhit.SetWidth(width);
  drawTrack.hits.push_back(dhit);
  ...
  drawTrack.SetStatWidth(width);
  drawTrack.DrawGlobal();
//  drawTrack.DrawHitsGlobal();
}

void AliHLTTPCCADisplay::SpecDrawLineG( float x1, float y1, float z1, float x2, float y2, float z2, int color, float width )
{
  double vx[2], vy[2], vz[2], vr[2];
  vx[0] = x1 * fCos + y1 * fSin;
  vy[0] = y1 * fCos - x1 * fSin;
  vx[1] = x2 * fCos + y2 * fSin;
  vy[1] = y2 * fCos - x2 * fSin;
  vz[0] = z1;
  vz[1] = z2;
  vr[0] = sqrt( vx[0]*vx[0] + vy[0]*vy[0] );
  vr[1] = sqrt( vx[1]*vx[1] + vy[1]*vy[1] );
  static TPolyLine pl;
  pl.SetLineColor( color );
  pl.SetLineWidth( width );
  fYX->cd();
  pl.DrawPolyLine( 2, vx, vy );
  fZX->cd();
#ifndef RZ
  pl.DrawPolyLine( 2, vz, vy );
#else
  pl.DrawPolyLine( 2, vz, vr );
#endif
}

void AliHLTTPCCADisplay::SpecDrawLineZ( float x1, float y1, float z1, int slice1, float x2, float y2, float z2, int slice2, int color, float width )
{
  TDrawTrack drawTrack;
  TDrawHit dhit;
  dhit.x = x1;
...
//if( hit.ISlice() != 22 && hit.ISlice() != 21 ) continue;
    ...
    drawTrack.hits.push_back(dhit);
#if 0
dhit.SetColor(kOrange);
dhit.SetWidth(width*3);
if( i == 0 ) dhit.DrawGlobal();

if( i == 0 ) {
    TDrawHit dhit1;
    double xh(hit.X()), yh(hit.Y()), zh(hit.Z());
    const float cA = trpar.GetCosPhi();
    const float sA = trpar.GetSinPhi();
    double xh0 = trpar.GetX();//hit.X()*cA +  hit.Y()*sA;
    double yh0 = trpar.GetY();//-hit.X()*sA +  hit.Y()*cA;
    Global2View( xh0, yh0, &xh, &yh );
    //
    float dca_x, dca_y, dca_z;
    trpar.GetDCAPoint( 0., 0., 0., dca_x, dca_y, dca_z, 0.00149691 );
    //
    dhit1.x = xh;//trpar.GetX();
    dhit1.y = yh;//trpar.GetY();
    dhit1.z = trpar.GetZ();
    dhit1.alpha = TDrawHit::GetAlpha( fGB, hit.ISlice() );
    dhit1.SetColor(kGreen);
    dhit1.SetWidth(1);
    dhit1.DrawGlobal();
    ...
    dca.y = dca_y;
    dca.z = dca_z;
    dca.alpha = TDrawHit::GetAlpha( fGB, hit.ISlice() );
    dca.SetColor(kBlack);
    dca.SetWidth(0.5);
    dca.DrawGlobal();
//    Ask();
